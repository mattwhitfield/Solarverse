...
namespace Solarverse
{
    ...
    public partial class MainWindow : Window, ITimeSeriesHandler
    {
        ...
        private void AddFillBetween(Plot plot, TimeSeries series, Func<TimeSeriesPoint, double?> value1, Func<TimeSeriesPoint, double?> value2, Func<double, bool> include, Func<DateTime, bool> timeFilter, Color color)
        {
...
//            AddSteppedPlot(WpfPlot1.Plot, series, x => x.ExcessPowerKwh, Color.Green, false, x => x > maxTimeWithPvActual);
            ...
            if (StaticConfigurationProvider.Configuration.TestMode)
            {
                AddPlot(WpfPlot3.Plot, series, x => (x.RequiredBatteryPowerKwh / StaticConfigurationProvider.Configuration.Battery.CapacityKwh) * 100, Color.DarkBlue, true, x => x >= maxTimeWithPvActual).MarkerShape = MarkerShape.filledCircle;
                AddPlot(WpfPlot3.Plot, series, x => (x.MaxCarryForwardChargeKwh / StaticConfigurationProvider.Configuration.Battery.CapacityKwh) * 100, Color.MediumPurple, true, x => x >= maxTimeWithPvActual).MarkerShape = MarkerShape.filledCircle;
            }
